...
class DefaultInformationExtractionService(BaseInformationExtractionService[TChunk, TEntity, TRelation, GTId]):
  ...
  def extract(
    ...
    return [asyncio.create_task(self._extract(llm, document, prompt_kwargs, entity_types)) for document in documents]
  ...
  async def _extract_from_chunk(
    ...
    try:
      ...
      logger.error(f"Error during information extraction from chunk: {e}")
      return None
    ...
    try:
      ...
      for graph in graphs:
        try:
          ...
          logger.error(f"Error during graph upsert: {e}")
          continue
    finally:
